...
public class NewsBizImpl implements NewsBiz {
	...
	public List query_news(Topic topic) throws Exception {
		...
		try {
			...
			Object[] params = null;
			if(topic==null) {
				sql = "select nid,ntid,ntitle,nauthor,ncreatedate,npicpath,ncontent,nmodifydate,nsummary from news order by nid asc";
				//参数封装(用户名)
				params = new Object[]{};
			}
			else {
				//sql
				sql = "select nid,ntid,ntitle,nauthor,ncreatedate,npicpath,ncontent,nmodifydate,nsummary from news where ntid=? order by nid asc";
				//参数封装(用户名)
